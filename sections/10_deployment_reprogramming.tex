...
\begin{diagram}
   Identify target ECU (from Machine IR / Vehicle Knowledge Graph)
             |
             v
   Determine supported programming path (see 10.2 -- must be a
   documented/authorized mechanism for THIS ECU; if none exists,
   go to 10.3 Legacy Adapter path instead)
             |
             v
   Create/update Machine IR entry for the replacement behavior
             |
             v
   Generate firmware (Section 8 pipeline, targeting either a
   ...
   SIL  (simulate against Machine IR model of the vehicle)
             |
             v
   HIL / bench validation (Alfred Lab hardware, Section 13;
   ...
   certification evidence bundle, Section 8)
             |
             v
   Program ECU (mechanism per 10.2)
             |
             v
   Verify (read-back / checksum verification; functional smoke
   test via Active Test Mode allowlist, Section 9)
             |
             v
...
\begin{diagram}
   Original ECU remains in place, unmodified
             |
             v
   Alfred Adapter / Gateway (Section 10.4) -- observes and, where
   validated (Section 9), issues commands through the ECU's own
