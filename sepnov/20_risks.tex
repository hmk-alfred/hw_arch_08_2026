...
Five PCB variants (Rev~A$\times$2, Rev~B$\times$3) in a 12-week window & More board programs than the prior single-common-node plan, which trades some schedule margin for the correctness of not forcing incompatible silicon onto one board. Mitigation: Rev~A-Control/Light share layout/BOM/review effort tightly (Section~\ref{sec:platform-strategy}) to recover some of that margin; Rev~C is the explicit shock absorber if schedule slips (Section~\ref{sec:feature-cuts}). \\
...
STM32G4 FDCAN peripheral/driver maturity (Gateway only) & Same risk as the prior plan: integrated FDCAN shifts risk into MCU peripheral driver correctness. Mitigation unchanged: bring up FDCAN on a Nucleo board in parallel with Rev~B-Gateway schematic capture, not serially after. \\
